...
PLazyInternalBlock::PLazyInternalBlock(PBlock* parent, int64_t start_index, int64_t end_index, int r, int leaf_length, std::string& source, int child_number):
        PBlock(parent, start_index, end_index, r, leaf_length, source, child_number) {
}

PLazyInternalBlock::PLazyInternalBlock(PBlock* parent, int64_t start_index, int64_t end_index, int r, int leaf_length, std::basic_string<int64_t>& source, int child_number):
        PBlock(parent, start_index, end_index, r, leaf_length, source, child_number) {
}

PLazyInternalBlock::~PLazyInternalBlock() {
    for (int i = children_.size()-1 ; i>=0; i--)
        delete children_[i];
}


int PLazyInternalBlock::add_rank_select_support(int c) {
    int r = 0;
...
int64_t PLazyInternalBlock::add_differential_access_support() {
    int64_t r = 0;
    for (PBlock* child: children_) {
        child->prefix_sum_ = r;
...
int PLazyInternalBlock::test_rank(int c, int i, int& counter) {
    ++counter;
...
int64_t PLazyInternalBlock::differential_access(int i) {
    int cumulative_length = 0;
    for (PBlock* child: children_) {
        cumulative_length += child->length();
...
int PLazyInternalBlock::rank(int c, int i) {
    int cumulative_length = 0;
    for (PBlock* child: children_) {
        cumulative_length += child->length();
...
int64_t PLazyInternalBlock::differential_access_alternative(int i) {
    int cumulative_length = 0;
    int64_t r = 0;
    for (PBlock* child: children_) {
        cumulative_length += child->length();
...
int PLazyInternalBlock::rank_alternative(int c, int i) {
    int cumulative_length = 0;
    int r = 0;
    for (PBlock* child: children_) {
        cumulative_length += child->length();
...
int64_t PLazyInternalBlock::better_differential_access(int i) {
    int cumulative_length = 0;
    for (PBlock* child: children_) {
        cumulative_length += child->length();
...
int PLazyInternalBlock::better_rank(int c, int i) {
    int cumulative_length = 0;
    for (PBlock* child: children_) {
        cumulative_length += child->length();
...
int PLazyInternalBlock::select(int c, int j) {
    int cumulative_length = 0;
    int r = 0;
...
int PLazyInternalBlock::select_alternative(int c, int j) {
    int cumulative_length = 0;
    int r = 0;
    for (auto it = children_.begin(); it != children_.end(); ++it) {
...
int PLazyInternalBlock::better_select(int c, int j) {
    int cumulative_length = 0;
    int r = 0;
    for (PBlock* child: children_) {
...
void PLazyInternalBlock::print() {
    std::cout << " (" << start_index_ << "," << end_index_ << ") " ;
}


std::vector<PBlock*>& PLazyInternalBlock::children() {
    if (children_.size() == 0) {
        ...
        if (next_length <= leaf_length_) {
            ...
            for (int i = 0; i < r_; ++i) {
                ...
                int end = start_index_ + (i + 1) * next_length - 1;
                if (init < source_.size()) {
                    PBlock *child = new PLazyInternalBlock(this, init, end, r_, leaf_length_, source_, i);
                    children_.push_back(child);
                }
            }
        }
...
std::vector<PBlock*>& PLazyInternalBlock::wchildren() {
    if (children_.size() == 0) {
        int next_length = length()/r_;
        if (next_length <= leaf_length_) {
            for (int i = 0; i < r_; ++i) {
            ...
            for (int i = 0; i < r_; ++i) {
                ...
                int end = start_index_ + (i + 1) * next_length - 1;
                if (init < wsource_.size()) {
                    PBlock *child = new PLazyInternalBlock(this, init, end, r_, leaf_length_, wsource_, i);
                    children_.push_back(child);
                }
            }
        }
...
void PLazyInternalBlock::put_child(int i, PBlock* b) {
...
int PLazyInternalBlock::number_of_nodes() {
    int c = 1;
    for (PBlock* child: children_)
...
int PLazyInternalBlock::number_of_back_blocks(){
    int c = 0;
    for (PBlock* child: children_)
...
int PLazyInternalBlock::number_of_internal_blocks() {
    int c = 1;
    for (PBlock* child: children_)
...
int PLazyInternalBlock::number_of_leaf_blocks() {
    int c = 0;
    for (PBlock* child: children_)
...
int PLazyInternalBlock::height() {
    return 1 + children_[0]->height();
}

int PLazyInternalBlock::number_of_leaves() {
    int c = 0;
    for (PBlock* child: children_)
...
void PLazyInternalBlock::clean_unnecessary_expansions() {
...
int PLazyInternalBlock::clean_unnecessary_expansions(int c) {
...
void PLazyInternalBlock::wclean_unnecessary_expansions() {
    for (std::vector<PBlock *>::reverse_iterator rit = children_.rbegin(); rit != children_.rend(); ++rit) {
        PBlock *b = (*rit);
...
int PLazyInternalBlock::wclean_unnecessary_expansions(int c) {
    int pointing_to_subtree = 0;
    for (std::vector<PBlock *>::reverse_iterator rit = children_.rbegin(); rit != children_.rend(); ++rit) {
        PBlock *b = (*rit);
...
bool PLazyInternalBlock::is_leaf() {
    return false;
}

int PLazyInternalBlock::access(int i) {
    int cumulative_length = 0;
    for (PBlock* child: children_) {
        cumulative_length += child->length();
...
int PLazyInternalBlock::waccess(int i) {
    int cumulative_length = 0;
    for (PBlock* child: children_) {
        cumulative_length += child->length();
...
int PLazyInternalBlock::access_2(int i, int& a) {
    int cumulative_length = 0;
    for (PBlock* child: children_) {
        cumulative_length += child->length();
...
int PLazyInternalBlock::number_of_selfreferences() {
    int n = 0;
    for (PBlock* child: children_)
...
bool PLazyInternalBlock::check_heuristic() {
    bool check = true;
    for (PBlock* child : children_)
...
bool PLazyInternalBlock::contains_double_pointer(bool from_back) {
...
int PLazyInternalBlock::compute_max_hop() {
    int max_hop = 0;
    for (PBlock* child: children_) {
...
int PLazyInternalBlock::add_fwdsearch_support() {
    int min_excess = 1;
    for (PBlock* child : children_) {
...
int PLazyInternalBlock::add_preffix_min_count_fields() {
    int min_excess = 1;
    int count = 0;
    for (PBlock* child : children_) {
...
int PLazyInternalBlock::add_max_fields() {
    int max_excess = -1;
    for (PBlock* child : children_) {
...
int PLazyInternalBlock::fwdsearch(int i, int d, int& e) {
    int a = -1;
    int cumulative_length = 0;
...
int PLazyInternalBlock::positive_fwdsearch(int i, int d, int& e) {
    int a = -1;
    int cumulative_length = 0;
    bool first_search_done = false;
    for (PBlock* child: children_) {
...
int PLazyInternalBlock::bwdsearch(int i, int d, int& e) {
...
int PLazyInternalBlock::positive_bwdsearch(int i, int d, int& e) {
    int a = source_.length();
    int cumulative_length = 0;
    for (PBlock* child: children_) {
        cumulative_length += child->length();
...
int PLazyInternalBlock::min_select(int i, int j, int &t, int &e, int m) {
...
int PLazyInternalBlock::min_count(int i, int j, int& e, int& m) {
...
int PLazyInternalBlock::min_excess(int i, int j, int& e) {
    if (i == 0 && (j == length()-1 || j == source_.length() - start_index_ - 1)) {
        for (auto pair : ranks_) {
            if (pair.first == source_[0]) e += pair.second;
            else e -= pair.second;
...
int PLazyInternalBlock::max_excess(int i, int j, int& e) {
    if (i == 0 && (j == length()-1 || j == source_.length() - start_index_ - 1)) {
        for (auto pair : ranks_) {
            if (pair.first == source_[0]) e += pair.second;
            else e -= pair.second;
...
int PLazyInternalBlock::add_rank_select_leaf_support() {
...
int PLazyInternalBlock::leaf_rank(int i) {
    int cumulative_length = 0;
    for (PBlock* child: children_) {
        cumulative_length += child->length();
...
int PLazyInternalBlock::leaf_rank_alternative(int i) {
    int cumulative_length = 0;
    int r = 0;
    for (PBlock* child: children_) {
        cumulative_length += child->length();
...
int PLazyInternalBlock::better_leaf_rank(int i) {
    int cumulative_length = 0;
    for (PBlock* child: children_) {
        cumulative_length += child->length();
...
int PLazyInternalBlock::leaf_select(int j) {
    int cumulative_length = 0;
    int r = 0;
    for (auto it = children_.begin(); it != children_.end(); ++it) {
...
int PLazyInternalBlock::leaf_select_alternative(int j) {
    int cumulative_length = 0;
    int r = 0;
    for (auto it = children_.begin(); it != children_.end(); ++it) {
...
int PLazyInternalBlock::better_leaf_select(int j) {
    int cumulative_length = 0;
    int r = 0;
    for (PBlock* child: children_) {
...
int PLazyInternalBlock::test_fwdsearch(int i, int d, int& e, int& c) {
    ++c;
    int a = -1;
    int cumulative_length = 0;
    bool first_search_done = false;
...
int PLazyInternalBlock::test_fwdsearch(int i, int j, int d, int& e, int& c) {
    ...
    for (PBlock* child: children_) {
        ...
        if (!first_search_done) {
            ...
            if (j  < cumulative_length) {
                ...
                a = child->test_fwdsearch(-1, child->length()-1, d, e,c);
                if (a != -1) {
                    a += cumulative_length - child->length();
                    break;
                }
...
int PLazyInternalBlock::test_bwdsearch(int i, int d, int& e, int& c) {
    ++c;
    int a = source_.length();
    int cumulative_length = 0;
    for (PBlock* child: children_) {
...
int PLazyInternalBlock::test_bwdsearch(int i, int j, int d, int& e, int& c) { //Let's supposse i<=j
    ++c;
    int a = source_.length();
    int cumulative_length = 0;
    for (PBlock* child: children_) {
    ...
    for (auto rit = children_.rbegin(); rit != children_.rend(); rit++) {
        ...
        if (!first_search_done) {
            if (j >= cumulative_length) {
                if (i >= cumulative_length) {
                    a = child->test_bwdsearch(i - cumulative_length, j - cumulative_length, d, e,c);
                    if (a != source_.length()) a += cumulative_length;
                    break;
                }
...
int PLazyInternalBlock::test_min_excess(int i, int j, int& e, int& c) {
...
void PLazyInternalBlock::put_fix_references() {
