...
LazyInternalBlock::LazyInternalBlock(Block* parent, int start_index, int end_index, int r, int max_leaf_length, std::string& source, int child_number, int max_length_level_block):
        Block(parent, start_index, end_index, r, max_leaf_length, source, child_number) {
    next_max_length_level_block_ = (max_length_level_block % r_) ? max_length_level_block/r_ + 1 : max_length_level_block/r_;
}

LazyInternalBlock::~LazyInternalBlock() {
    for (int i = children_.size()-1 ; i>=0; i--)
        delete children_[i];
}

int LazyInternalBlock::add_rank_select_leaf_support() {
...
int LazyInternalBlock::add_rank_select_support(int c) {
    int r = 0;
    for (Block* child: children_) {
...
int LazyInternalBlock::rank(int c, int i) {
...
int LazyInternalBlock::rank_alternative(int c, int i) {
    int cumulative_length = 0;
    int r = 0;
    for (Block* child: children_) {
        cumulative_length += child->length();
...
int LazyInternalBlock::better_rank(int c, int i) {
    int cumulative_length = 0;
    for (Block* child: children_) {
        cumulative_length += child->length();
...
int LazyInternalBlock::select(int c, int j) {
    int cumulative_length = 0;
    int r = 0;
...
int LazyInternalBlock::select_alternative(int c, int j) {
    int cumulative_length = 0;
    int r = 0;
    for (auto it = children_.begin(); it != children_.end(); ++it) {
...
int LazyInternalBlock::better_select(int c, int j) {
    int cumulative_length = 0;
    int r = 0;
    for (Block* child: children_) {
...
int LazyInternalBlock::leaf_rank(int i) {
    int cumulative_length = 0;
    for (Block* child: children_) {
        cumulative_length += child->length();
...
int LazyInternalBlock::leaf_rank_alternative(int i) {
    int cumulative_length = 0;
    int r = 0;
    for (Block* child: children_) {
        cumulative_length += child->length();
...
int LazyInternalBlock::better_leaf_rank(int i) {
    int cumulative_length = 0;
    for (Block* child: children_) {
        cumulative_length += child->length();
...
int LazyInternalBlock::leaf_select(int j) {
    int cumulative_length = 0;
    int r = 0;
    for (auto it = children_.begin(); it != children_.end(); ++it) {
...
int LazyInternalBlock::leaf_select_alternative(int j) {
    int cumulative_length = 0;
    int r = 0;
    for (auto it = children_.begin(); it != children_.end(); ++it) {
...
int LazyInternalBlock::better_leaf_select(int j) {
    int cumulative_length = 0;
    int r = 0;
    for (Block* child: children_) {
...
void LazyInternalBlock::print() {
    std::cout << " (" << start_index_ << "," << end_index_ << ") " ;
}


std::vector<Block*>& LazyInternalBlock::children() {
    if (children_.size() == 0) {
        bool children_leaves = next_max_length_level_block_ <= max_leaf_length_ || next_max_length_level_block_ < r_;
        if (children_leaves) {
            ...
            for (i = 0; i < number_of_big_blocks; ++i) {
                int init = start_index_ + i * hop;
                int end = start_index_ + (i + 1) * hop - 1;
                Block *child = new LazyInternalBlock(this, init, end, r_, max_leaf_length_, source_, i, next_max_length_level_block_);
                children_.push_back(child);
            }
            int start_of_small_blocks = start_index_ + number_of_big_blocks * hop;
            --hop;
            ...
            for (int j = 0; j < r_ - number_of_big_blocks; ++j) {
                int init = start_of_small_blocks + j * hop;
                int end = start_of_small_blocks + (j + 1) * hop - 1;
                Block *child = new LazyInternalBlock(this, init, end, r_, max_leaf_length_, source_, j + i, next_max_length_level_block_);
                children_.push_back(child);
            }
        }
    }
...
void LazyInternalBlock::put_child(int i, Block* b) {
...
int LazyInternalBlock::number_of_nodes() {
    int c = 1;
    for (Block* child: children_)
...
int LazyInternalBlock::number_of_back_blocks(){
    int c = 0;
    for (Block* child: children_)
...
int LazyInternalBlock::number_of_internal_blocks() {
    int c = 1;
    for (Block* child: children_)
...
int LazyInternalBlock::number_of_leaf_blocks() {
    int c = 0;
    for (Block* child: children_)
...
int LazyInternalBlock::height() {
    return 1 + children_[0]->height();
}

int LazyInternalBlock::number_of_leaves() {
    int c = 0;
    for (Block* child: children_)
...
void LazyInternalBlock::clean_unnecessary_expansions() {
...
int LazyInternalBlock::clean_unnecessary_expansions(int c) {
...
bool LazyInternalBlock::is_leaf() {
    return false;
}

int LazyInternalBlock::access(int i) {
    int cumulative_length = 0;
    for (Block* child: children_) {
        cumulative_length += child->length();
...
int LazyInternalBlock::access_2(int i, int& a) {
    int cumulative_length = 0;
    for (Block* child: children_) {
        cumulative_length += child->length();
...
int LazyInternalBlock::number_of_selfreferences() {
    int n = 0;
    for (Block* child: children_)
...
bool LazyInternalBlock::check_heuristic() {
    bool check = true;
    for (Block* child : children_)
...
bool LazyInternalBlock::contains_double_pointer(bool from_back) {
...
int LazyInternalBlock::compute_max_hop() {
    int max_hop = 0;
    for (Block* child: children_) {
...
void LazyInternalBlock::put_fix_references() {
